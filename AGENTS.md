...
- **Hosted ownership.** `host.rs` owns each live `Simulation` and its roster on
  one worker. Local channels, GNS UDP and HTTP submit typed requests through `HostHandle`; never
