...
- **Hosted ownership.** `host.rs` owns each live `Simulation` and its roster on
  one worker. TCP and HTTP submit typed requests through `HostHandle`; never
...
- **Physics.** Projectiles are rapier dynamic balls with CCD; armor housings
