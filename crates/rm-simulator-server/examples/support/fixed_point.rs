...
//! Quantization assumptions for the binary experiment, not live rule constants.
...
pub fn checkpoint(value: &mut Value, errors: &mut Errors, mode: Quantization) {
    ...
    let compact = &mut value["CompactSnapshot"];
    chassis(&mut compact["state"]["field"]["chassis"], errors);
    ...
    for projectile in compact["projectiles"].as_array_mut().unwrap() {
        round(
            &mut projectile[3],
            position_scale,
            position_bits,
...
pub fn normalize(message: &mut ServerMessage) {
